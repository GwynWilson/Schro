...
class Schrodinger(object):
    ...
    def mod_square(self):
        self.mod = self.psi_x * np.conj(self.psi_x)
        return np.real(self.mod)

    def tunnel_prob(self, end):
        idx = np.abs(self.x - end).argmin()
        return simps(self.mod_square()[idx:], self.x[idx:])  # Integrates from beyond the end point of the barrier
    ...
    def energy(self):
        ...
        self.mod_square()
        x_sp = self.mod * np.asarray(self.V_x)
...
while S.t < 120:
    S.time_step(dt, Nsteps=N_steps)

print(t_theory2(a, V0, E, m=m, hbar=hbar))
print(S.tunnel_prob(a))
